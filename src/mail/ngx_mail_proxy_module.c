...
ngx_mail_proxy_sieve_handler(ngx_event_t *rev)
{
    ...
    case ngx_sieve_start:
        ...
        ngx_log_debug2(NGX_LOG_DEBUG_MAIL, rev->log, 0, "!!!!mail proxy login '%V' '%uz'", &s->login, s->login.len);
        ngx_log_debug2(NGX_LOG_DEBUG_MAIL, rev->log, 0, "!!!!mail proxy login '%s' '%uz'", s->login.data, s->login.len);
	...
	b64_userpass.data = ngx_pnalloc(c->pool, b64_userpass.len);
        ...
        p = ngx_cpymem(userpass.data, s->login.data, 0);
	*p++ = '\0';
        p = ngx_cpymem(p, s->login.data, s->login.len);
        *p++ = '\0';
        p = ngx_cpymem(p, s->passwd.data, s->passwd.len);
        ngx_log_debug2(NGX_LOG_DEBUG_MAIL, rev->log, 0, "!!!!mail proxy PLAIN '%V' '%uz'", &userpass, userpass.len);
        ngx_log_debug2(NGX_LOG_DEBUG_MAIL, rev->log, 0, "!!!!mail proxy PLAIN '%s' '%uz'", userpass.data, userpass.len);
	ngx_encode_base64(&b64_userpass, &userpass);
        ngx_log_debug2(NGX_LOG_DEBUG_MAIL, rev->log, 0, "!!!!mail proxy BASE64 '%V' '%uz'", &b64_userpass, b64_userpass.len);
        ngx_log_debug2(NGX_LOG_DEBUG_MAIL, rev->log, 0, "!!!!mail proxy BASE64 '%s' '%uz'", b64_userpass.data, b64_userpass.len);
...
ngx_mail_proxy_read_response(ngx_mail_session_t *s, ngx_uint_t state)
{
    ...
    case NGX_MAIL_SIEVE_PROTOCOL:
    ngx_log_error(NGX_LOG_INFO, s->connection->log, 0, "upstream sent: \"%s\"", p);
        if (p[0] == '"') {
            /* multiline reply, check if we got last line */

            m = b->last - (sizeof(CRLF "OK" CRLF) - 1);
    ngx_log_error(NGX_LOG_INFO, s->connection->log, 0, "upstream m: \"%s\"", m);
    ...
    ngx_log_error(NGX_LOG_INFO, s->connection->log, 0, "upstream p: \"%s\"", p);
        switch (state) {
